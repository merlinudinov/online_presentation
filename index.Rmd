...
---
#1. Introduction
so the basic idea of this paper is to test the long-distance phenomena via experiments.

---
#1. Introduction
## Crucial Term Definitions
###1. **First-Order NonAdjacent Dependency**
only consonants are skipped
###2. **Second-Order NonAdjacent Dependency**
some vowels are also skipped based on the first one
---
#1. Introduction
## Crucial Term Definitions
###3. neutral vowel
a vowel that does not undergo vowel harmony
###4. transparent neutral vowel (denoted as $V^T$) 
a **neutral vowel** that allows the feature-agreement process to 'skip' itself.
[$V_{+}$$V_{-}^{T}$$V_{+}$] [second-order]

Ambiguity created by second-order leads to chanllenges for a learner [$V_{-}$$V_{-}^{T}$$V_{-}$] [second-order]
###5. opaque neutral vowel (denoted as $V^O$)
a **neutral vowel** that does not allow the feature-agreement process to 'skip' itself.
[$V_{+}$$V_{-}^{O}$$V_{-}$] [first-order]
---
#1. Introduction
## distal constratint <span style="font-variant: small-caps;">Distal*</span>
violated by [+back]X[-back] or [-back]X[+back]

the number of segments between two backs matter, 

---
#1. Introduction
## Hungarian
Hungarian provides a classic example of transparent vowels in vowel harmony.

the first vowel of the word determines the back feature of all following vowels, including the vowel features of suffixes that attach to the stem. (Finley 2015:49)

The front vowels [i] and [e] are transparent
[$pɔ_{+}lle:_{-}^{T}r-nɔ_{+}k$] ‘foreman-dat’

---
#1. Introduction
adults exposed to vowel harmony for the first time, should start with no harmony constraints
initial state , harmonic 50%, disharmonic 50%

harmonic percentage will go up after gradual learning.

between transparent and opaque, opaque 50%, transparent 25%? this is not right

in sum, they think opaque case is easier to learn, should require less traning to acquire
---
#1. Introduction
...
---
#3. Methods
aritificial grammar learning paradigm to test hypothesis [more like a simplified language model, cuz in real language you need to care about too many factors]

two-alternative forced-choice: basically, after training, there are two choices (the right one and the wrong one), you have to pick one. If. you picked the right one, that means you have learnt the pattern.

h1: opaque neutral vowels (first-order nonadjacency) should be easier to learn than transparent neutral vowels (second-order nonadjacency)
...
#3. Methods Summary
## Experiments Summary
###1. learning the behaviour of neutral vowels with a first-order nonadjacent pattern is compared to learning a second-order nonadjacent pattern p53
###2. they are trained first, and then tested, if there is no statistical difference, then we say they learnt, if there is statistical difference, (the test group scores less), then we say they have not learnt.
###3.
###4a
###4b
###4c
...
mainly the idea is that the participant trained on combination in training comibination above and they were tested on learnt/unlearnt patterns and got a score. The test is to pick the 'right' one in two choices.
---
#4. Experiment I
##**Methods**
...
Additionally, I really do not like their ways of displaying tables!!!!
They wanted to do too much stuff!
---
class: center, middle
